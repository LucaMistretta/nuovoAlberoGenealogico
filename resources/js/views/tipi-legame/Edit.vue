<template>
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Modifica Tipo di Legame</h1>
        
        <form v-if="store.tipoLegame" @submit.prevent="handleSubmit" class="max-w-2xl space-y-4">
            <div>
...
onMounted(() => {
    store.fetchTipoLegame(route.params.id);
});

watch(() => store.tipoLegame, (tipo) => {
    if (tipo) {
        form.value = {
            nome: tipo.nome || '',
...
const handleSubmit = async () => {
    loading.value = true;
    const result = await store.updateTipoLegame(route.params.id, form.value);
